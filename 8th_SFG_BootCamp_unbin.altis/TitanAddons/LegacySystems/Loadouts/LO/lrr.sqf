		...
		switch (_plastr) do {
			...
			default {
				...
				if (_selectuni == "mtp") then {
					...
					removeAllItems player;
					removeAllAssignedItems player;
					player addHeadgear "Booniehat_daguet_hs";
					player addGoggles "milgp_f_face_shield_shades_shemagh_MC";
					player addBackpack "SOG_mark";
...
if(local player) then { //	AB HIER WIRD STANDART AUSRÜSTUNG FESTGELEGT!
	if (!_rearm) then { // Hier alles was kein Verbrauchsgut ist
		...
		//Items to Uniform
		player addItemToUniform "muzzle_snds_B_arid_F";
	...
	//Vest REARM
	for "_i" from 1 to 5 do {player addItemToVest "ACE_20Rnd_762x51_Mag_Tracer";};
	for "_i" from 1 to 2 do {player addItemToVest "ACE_20Rnd_762x51_Mag_Tracer_Dim";};
	for "_i" from 1 to 5 do {player addItemToVest "ACE_20Rnd_762x51_M118LR_Mag";};
